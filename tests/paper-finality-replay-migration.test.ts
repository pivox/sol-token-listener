...
void test('adds replay-safe finalized receipts and an index-backed bounded cursor scan',async(context)=>{
  ...
  try{
    ...
    assert.deepEqual((await pool.query(`SELECT signature,observed_slot::text AS observed_slot,
      ...
      FROM chain_transaction_finality_replay_receipts`)).rows,[{
    ...
    await assert.rejects(pool.query(sql),/finalized replay receipt conflicts/u);
    await pool.query(`UPDATE chain_transaction_finality_replay_receipts
      SET immutable_fingerprint=$2 WHERE signature=$1`,[
    ...
    ) SELECT 'noise-'||value,'pumpfun','pump','noise-mint','noise-signature-'||value,
      5,value,0,'confirmed',$1,1,'{}'::jsonb FROM generate_series(1,100000) value`,[
      new Date(900),
    ]);
    await pool.query(`INSERT INTO raw_chain_events (
      event_id,source,program,mint,signature,slot,transaction_index,instruction_index,
      confirmation_status,observed_at,payload_version,payload
    ) SELECT 'relevant-'||value,'pumpfun','pump','relevant-mint',
      'relevant-signature-'||value,10,value,0,'confirmed',$1,1,'{}'::jsonb
      FROM generate_series(1,16) value`,[new Date(900)]);
    await pool.query('ANALYZE raw_chain_events');
    const explained=await pool.query(`EXPLAIN (FORMAT JSON,COSTS OFF)
      ${paperFinalityRelevantRawSql('parameters')}`,[
      'relevant-mint','relevant-16',4_097,
    ]);
    const planNodes=collectPlanNodes(explained.rows[0]);
    assert.ok(planNodes.some((node)=>
