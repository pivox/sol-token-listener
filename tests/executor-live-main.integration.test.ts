...
const preflightDraftSpecificationUrl = new URL(
  '../docs/superpowers/specs/2026-09-05-executor-preflight-draft-design.md',
  import.meta.url,
);
const preflightSourceSpecificationUrl = new URL(
...
void test('documents H2d-H2k external evidence without starting a canary',
  ...
  assert.equal(
    packageJson.scripts?.['executor:preflight-draft:start'],
    'node dist/src/preflight-draft/main.js',
  );
  assert.equal(
  ...
  assert.match(systemOverview, /La migration 052 est le head/u);
  assertContainsExactlyOnce(
    pumpFunArchitecture,
  ...
  assertContainsExactlyOnce(
    systemOverview,
    '<meta name="doc-version" content="1.1.4">',
  ...
  assertContainsExactlyOnce(
    ...
    'preflight draft specification version',
  );
  assertContainsExactlyOnce(
    preflightDraftSpecification,
    '**Version de la spécification parente :** 1.11.13',
    'preflight draft parent specification version',
  );
  assertContainsExactlyOnce(
    preflightSourceSpecification,
    '**Version de spécification :** 1.0.0',
    'preflight source specification version',
  );
  assertContainsExactlyOnce(
    preflightSourceSpecification,
  ...
  assert.equal((deploymentSmoke.match(/'051_transaction_inbox_decoder_quarantine_recovery\.sql'/gu)
    ?? []).length, 1);
  assert.equal((deploymentSmoke.match(/'052_transaction_inbox_urgent_fairness\.sql'/gu)
    ?? []).length, 1);
  assert.equal((deploymentSmoke.match(/'053_transaction_inbox_worker_admission_foundation\.sql'/gu)
    ?? []).length, 1);
  assert.equal((deploymentSmoke.match(/'054_paper_entry_boundary\.sql'/gu) ?? []).length, 1);
  assert.equal((deploymentSmoke.match(/'055_creation_entry_single_active_session\.sql'/gu)
    ?? []).length, 1);
