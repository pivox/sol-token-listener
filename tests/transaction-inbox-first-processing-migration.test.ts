...
void test('migration 050 classifies all legacy inbox rows unavailable without inventing evidence', async (context) => {
  ...
  try {
    ...
    // The current repository reads the later 052 scheduler field and writes the inert 053
    // admission field; this test deliberately keeps the remaining inbox schema at 049 so
    // migration 050 can still classify genuine legacy rows.
    ...
    await pool.query(`ALTER TABLE chain_transaction_inbox
      ADD COLUMN worker_admitted_at TIMESTAMPTZ`);
