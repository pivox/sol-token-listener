...
type ClaimOptions =
  ...
  | Readonly<{
      readonly ownerId: string;
      readonly leaseMs: number;
      readonly purpose: 'LIVE_RECOVER';
      readonly side?: 'BUY' | 'SELL';
    }>
  | Readonly<{
      readonly ownerId: string;
      readonly leaseMs: number;
      readonly purpose: 'CONFIRM' | 'RECONCILE' | 'EXECUTE' | 'DRY_RUN';
    }>;
...
void test('execution intent repository is an allowlisted domain-only persistence boundary', async () => {
  ...
  assert.equal(sourceFile.statements.length, 10);
...
function compileTimeNegativeAssertions(): void {
  ...
  void repository.claim({ ownerId: 'worker', leaseMs: 1, purpose: 'LIVE_RECOVER', side: 'OTHER' });
