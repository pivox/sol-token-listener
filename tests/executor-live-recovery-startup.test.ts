...
void test('pins every migration through 051 to a non-placeholder sha256', async () => {
  assert.equal(LIVE_RECOVERY_MIGRATION_CATALOG.length, 51);
  assert.equal(
...
void test('validates role, exact migration history, generation and open-work affinity in order', async () => {
  ...
  assert.deepEqual(evidence, {
    ...
    migrationHead: '051_transaction_inbox_decoder_quarantine_recovery.sql',
