...
void test('la migration crée une outbox append-only publique, indexée et sans FK parent', async () => {
  ...
  assert.match(sql, /CREATE INDEX IF NOT EXISTS api_event_stream_mint_sequence_idx[\s\S]*\(mint, sequence\)/u);
  assert.match(sql, /CREATE INDEX IF NOT EXISTS api_event_stream_purge_after_idx[\s\S]*\(purge_after\)/u);
...
void test('la migration enfile les révisions publiques avec trigger rejouable et backfill idempotent', async () => {
  ...
  assert.match(sql, /ON CONFLICT \(stream_event_id\) DO NOTHING/u);
...
void test('la migration fonctionne en base réelle si TEST_DATABASE_URL est configurée', async (context) => {
  ...
  try {
    ...
    const revisions = await pool.query<{ readonly confirmation_status: string }>(
      'SELECT confirmation_status FROM api_event_stream WHERE domain_event_id = $1 ORDER BY sequence', ['domain-live'],
    );
