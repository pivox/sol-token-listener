...
const latestMigrationName = '053_transaction_inbox_worker_admission_foundation.sql';
...
void test('websocket health migration applies on an empty schema and records current migration 047', async (context) => {
  const databaseUrl = process.env.TEST_DATABASE_URL;
  if (databaseUrl === undefined || databaseUrl.trim() === '') {
    context.skip('TEST_DATABASE_URL absent: websocket health migration test skipped');
    return;
