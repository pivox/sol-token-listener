...
export const EXECUTION_READINESS_AUTHORITY_SQL = `SELECT
  ...
  EXISTS(SELECT 1 FROM migration_history
    WHERE version='045_execution_wallet_snapshot_refresh.sql')
    AS migration_045_present,
...
function validAuthority(row: Readonly<Record<string, unknown>> | undefined): boolean {
  return row !== undefined
    && sameKeys(row, [
      ...
      'membership_inherit', 'membership_set', 'migration_045_present',
    ...
    && row.migration_045_present === true
